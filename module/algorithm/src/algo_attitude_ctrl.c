...
{
	// 角度PID参数初始化
	pidInit(&att_r_pid, att_r_pid_param);
	pidInit(&att_p_pid, att_p_pid_param);
	pidInit(&att_y_pid, att_y_pid_param);
	// 角速度PID参数初始化
	pidInit(&rate_r_pid, rate_r_pid_param);
	pidInit(&rate_p_pid, rate_p_pid_param);
	pidInit(&rate_y_pid, rate_y_pid_param);
}
...
void acAttitudeCtrl(rctrl_t *act_control)
{
	...
	// 角速度期望值
	rate_r_pid.desired = rate_d[0] = pidCalculate(&att_r_pid);
	rate_p_pid.desired = rate_d[1] = pidCalculate(&att_p_pid);
	// rate_y_pid.desired = rate_d[2] = pidCalculate(&att_y_pid);
	rate_y_pid.desired = rate_d[2];

	// 电机PWM控制分量
	act_control->roll = pidCalculate(&rate_r_pid);
	act_control->pitch = pidCalculate(&rate_p_pid);
	act_control->yaw = pidCalculate(&rate_y_pid);
	
	acMixerCtrl(act_control);
}
...
void acMixerCtrl(rctrl_t *act_control)
{
	...
	{
		pwmSet(XTIM_CHANNEL_1, motor_pwm[0]);
		pwmSet(XTIM_CHANNEL_2, motor_pwm[1]);
		pwmSet(XTIM_CHANNEL_3, motor_pwm[2]);
		pwmSet(XTIM_CHANNEL_4, motor_pwm[3]);
	}
	else
	{
		pwmSet(XTIM_CHANNEL_4, motor_pwm[3]);
